...
| A4 | M | **Retry loop re-sends non-idempotent requests.** `$shouldRetry` is computed from status code only (429/5xx/0 incl. client timeout); POST/PATCH/DELETE are re-sent — duplicate campaign creation / decision resubmission risk. | `SP.ApiClient.psm1:455` |
| A5 | L | **`Build-SPQueryString` NREs on `$null` param value**, bypassing the normalized error envelope (call site is outside the retry try/catch). | `SP.ApiClient.psm1:114` |
...
| B5 | M | **Interrupted TTL-refresh permanently seals partial data.** Old items file deleted at fetch start, old meta replaced only at finalize; crash mid-fetch + campaign completes → partial set sealed permanent, no self-heal. | `SP.AuditQueries.psm1:7320, 7461` |
...
| B9 | L | Item memory cache keyed by campaign id only — ignores `-CachePath`; second path gets the other directory's items. | `SP.AuditQueries.psm1:7155` |
| B10 | L | Partial-fetch resume path ignores `-NoCache` — "fresh" fetch silently includes leftover partial disk data. | `SP.AuditQueries.psm1:7290` |
| B11 | L | PII ACL warning misses `FullControl` (`FileSystemRights -match 'Read'` doesn't match "FullControl"). | `SP.IdentityService.psm1:175` |
...
| D2 | M | Remediation proof only matches literal `REVOKE` — `REVOKED`/`DENY`/`REJECT`/`EXCEPTION` and nested `{value:...}` shapes dropped; Section 4 vs Section 6 totals contradict. | `SP.AuditReportCore.psm1:1609`; also `:928`, `Get-SPRevocationDisposition:83` |
| D3 | M | `Get-SPRemediationStatus` accepts source-only match → unrelated revocations marked "Provisioned", failed revokes hidden. | `SP.AuditQueries.psm1:2896` |
...
| D6 | M | `Sort-Object -Property` on hashtable elements is a no-op on PS 5.1 → `LastCampaign`/`LastCampaignDate` wrong in source-coverage report. | `SP.AuditQueries.psm1:2568` |
| D7 | M | Reviewer-delegation metric uses loop-leaked `$campaignCreated` (last campaign only) for all reviewers' `AvgHoursBeforeDelegation`. | `SP.AuditQueries.psm1:6466` |
| D8 | M | `Get-SPSourceAggregationHealth` mixes `[datetime]::UtcNow` with local-Kind parse — freshness off by the UTC offset (false/missed staleness). | `SP.AuditQueries.psm1:5471, 5563` |
| D9 | M | Empty (0-item) signed certs classified NotStarted and escalated on the leadership "stalled reviewers" list. | `SP.CampaignDiff.psm1:207, 219, 379` |
...
| D11 | L | `Measure-SPCampaignMetrics` doesn't unwrap nested decision objects → 0% completion on nested-shape tenants while audit shows approvals. | `SP.AuditReportCore.psm1:3021` |
...
| E2 | H | V5 baseline scope-added reads the wrong trend record when a day has multiple captures (`$trendRecords[$dailyData.IndexOf($d)]` misalignment). | `Invoke-SPDailyEvidenceReportV5.ps1:799` |
| E3 | H | V7 "suspect" filter silently drops every legitimately 100%-completed day by default; all-complete window → exit 5 "No calendar days resolved". | `Invoke-SPDailyEvidenceReportV7.ps1:421, 441` |
...
| E5 | M | Series chrono-key parses period tokens with current culture — wrong instance ordering (wrong baseline/newest) on dd/MM locales; whole V4c/V4d/V4e delta engine misclassifies. | `SP.AuditQueries.psm1:7726` |
| E6 | M | Orchestrator Steps 8/9 swallow `Success=$false` results — green "Success" line, exit 0, audit JSONL certifies a run that collected nothing. | `Invoke-SPDailyOrchestrator.ps1:1075, 1136` |
| E7 | M | Step 11 stalled-reviewer warnings never recorded in step results/audit trail; Step 11/12 warnings never raise the exit code. | `Invoke-SPDailyOrchestrator.ps1:1266, 1310, 1354` |
| E8 | M | V6 Chart 10 / V7 Chart 7 "CUMULATIVE/TOTALS" rows sum daily *snapshots*, not deltas — multiply-count when one campaign spans days. | `V6.ps1:1292`; `V7.ps1:1362` |
| E9 | M | Scraper `-Until` excludes reports from the Until day when filenames carry timestamps (time-of-day vs midnight comparison). | `Invoke-SPPendingReviewerScrape.ps1:255` |
| E10 | L | Scraper `-MinMisses` / chronic-pending denominator count per report file, not per calendar day — regenerated reports skew both. | `Invoke-SPPendingReviewerScrape.ps1:282` |
