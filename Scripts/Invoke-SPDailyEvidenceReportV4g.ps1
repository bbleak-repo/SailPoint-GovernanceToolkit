...
<#
.SYNOPSIS
    Generates the daily certification evidence report (v4g) -- v4 with persistent entitlement
    state database for accurate "Newly Decided" detection (output: daily-evidence-v4g-*.html).
.DESCRIPTION
    V4g is a fork of V4 that adds a persistent entitlement state database
    (entitlement-state.jsonl). This solves the "Newly Decided" accuracy problem: ISC
    auto-approves remaining items on force-close with decision="APPROVE" and
    comments="idNowAutoApproved", making two-campaign diffs unreliable. V4g tracks
    per-entitlement state across campaigns, distinguishing genuine reviewer decisions
    from auto-close artifacts.

    The entitlement state database uses four honest decision states:
      APPROVE   - genuine reviewer approval
      REVOKE    - genuine reviewer revocation
      PENDING   - ISC decision is null (unsigned reviewer, not yet committed)
      UNDECIDED - ISC auto-closed with idNowAutoApproved (reviewer never acted)

    Otherwise identical to V4: focused evidence report with pending reviewer accountability,
    revoked decisions, and newly approved access. Produces a compact action-oriented artifact:
      ...
      Decision Summary             Revoked register + Newly Decided (from state DB) + New Scope
      Entitlement State Summary    Counts by honest decision state (APPROVE/REVOKE/PENDING/UNDECIDED)

    V4g integrates the scope-diff engine (same as V3/V4) and the entitlement state database.
    Use -NoCapture to re-render from existing snapshots without capturing a new one.

    V4 remains available unchanged. Use V4g when you need accurate Newly Decided tracking.

    NAMING NOTE: this script briefly shipped as "V4c" (2026-07-16), overwriting the
    original read-only series-attestation V4c. Per the toolkit's sub-versioning
    convention it now lives at V4g and the original V4c is restored -- both exist.
...
.NOTES
    Script:  Invoke-SPDailyEvidenceReportV4g.ps1
    Version: 1.0.0
    Exit codes:
...
Write-Host '  Daily Evidence Report (v4g)' -ForegroundColor Cyan
...
Write-SPLog -Message "Invoke-SPDailyEvidenceReport started: CorrelationID=$correlationID DaysBack=$effectiveDaysBack" `
    -Severity INFO -Component 'DailyEvidenceV4g' -Action 'Start' -CorrelationID $correlationID
...
if ($isWhatIf) {
    ...
    Write-Host '  [1b] Scope-diff snapshot (newly approved access)' -ForegroundColor Gray
    Write-Host '  [ST] State Tracking: entitlement-state.jsonl + reviewer-state.jsonl' -ForegroundColor Gray
...
try {
    ...
    if ($currentCampaigns.Count -eq 0) {
        ...
        Write-SPLog -Message "No campaigns found in last $effectiveDaysBack day(s)" `
            -Severity WARN -Component 'DailyEvidenceV4g' -Action 'NoCampaigns' -CorrelationID $correlationID
        Write-Host ''
        exit 1
    }
    ...
    foreach ($campaign in $currentCampaigns) {
        ...
        try {
            ...
            Write-SPLog -Message "Campaign audit build failed for ${campName}: $($_.Exception.Message)" `
                -Severity WARN -Component 'DailyEvidenceV4g' -Action 'AuditBuildError' -CorrelationID $correlationID
        }
    }
    ...
    Write-SPLog -Message "Campaign fetch exception: $($_.Exception.Message)" `
        -Severity ERROR -Component 'DailyEvidenceV4g' -Action 'CampaignFetchError' -CorrelationID $correlationID
    $worstExitCode = 5
}
Write-Host ''
...
try {
    ...
    foreach ($audit in $campaignAudits) {
        ...
        # If DecisionDate is empty, fall back to a sign-off date ONLY while the campaign
        # is still ACTIVE: on a force-closed COMPLETED campaign ISC stamps every cert
        # SIGNED, so using SignOffDate there fabricates a genuine-looking decision time
        # for auto-approved items. The campaign Created fallback is flagged as
        # non-genuine by the decision-date provenance display downstream.
        $fallbackDate = [string]$audit['Created']
        $ra2 = $audit['ReviewerActions']
        $auditStatusUp = ([string]$audit['Status']).ToUpperInvariant()
        if ($auditStatusUp -notin @('COMPLETED', 'COMPLETING') -and $null -ne $ra2 -and $null -ne $ra2['Primary']) {
    ...
    Write-SPLog -Message "Scope-diff exception: $($_.Exception.Message)" -Severity ERROR -Component 'DailyEvidenceV4g' -Action 'ScopeDiffError' -CorrelationID $correlationID
}
Write-Host ''

#endregion

#region Resolve metrics path (shared by Step 1c + metrics write)
...
try {
    ...
    foreach ($audit in $campaignAudits) {
        ...
        # Reviewer breakdown. GENUINE sign-off ($signedCount / $rvCompPct) is computed
        # AFTER $rvItemCounts below -- it needs per-reviewer canonical pending counts
        # (a reviewer holding undecided items cannot have genuinely signed; their cert's
        # SIGNED phase is the ISC force-close, not the reviewer). Same doctrine as V4b.
        ...
        # reviewers with canonically-undecided items (the pending veto needs no signedBy
        # provenance). Distinct-by-reviewer numerator AND denominator.
        $pendingNamesJsonl = @($rvItemCounts.Keys | Where-Object { [int]$rvItemCounts[$_].Pending -gt 0 })
        $gsoJsonl = Get-SPGenuineReviewerSignOff -Reviewers $allReviewers -Roster @() -PendingReviewerNames $pendingNamesJsonl
        ...
        foreach ($rv in $allReviewers) {
            ...
            $reviewerRecords.Add([ordered]@{
                ...
                # GENUINE sign-off only: a SIGNED phase with undecided items is the
                # force-close, not the reviewer.
        ...
        $metricsRecord = [ordered]@{
            # schemaVersion 3 = canonical counts + GENUINE reviewer sign-off (same
            # contract V4b stamps; V7-family consumers gate suspect heuristics on >= 2).
            schemaVersion    = 3
...
#region State Tracking (Entitlement + Reviewer)

Write-Host '  State Tracking: Entitlement + Reviewer' -ForegroundColor Cyan
$stepStart = Get-Date
$tracking = $null
$priorStateSnapshot = @{}

try {
    $tracking = Invoke-SPStateTracking -MetricsPath $metricsPath -CampaignAudits $campaignAudits -TodayLabel $todayLabel
    ...
    Write-Host "    $rTag" -ForegroundColor Green
}
catch {
    $stepDuration = ((Get-Date) - $stepStart).TotalSeconds
    $stepResults['StateTracking'] = @{ Status = 'Warning'; Detail = $_.Exception.Message; Duration = [math]::Round($stepDuration, 2) }
    Write-Host "    WARN: $($_.Exception.Message)" -ForegroundColor Yellow
    Write-SPLog -Message "State tracking exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4g' -Action 'StateTrackingError' -CorrelationID $correlationID
}
...
try {
    ...
    Write-SPLog -Message "Campaign completion KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4g' -Action 'CompletionError' -CorrelationID $correlationID
...
try {
    ...
    Write-SPLog -Message "Overdue reviews KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4g' -Action 'OverdueError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "Revocations KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4g' -Action 'RevocationError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "Remediation SLA KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4g' -Action 'SlaError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "High-risk exposure KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4g' -Action 'HighRiskError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "Reviewer health KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4g' -Action 'ReviewerHealthError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "Confidence score exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4g' -Action 'ConfidenceError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
foreach ($audit in $campaignAudits) {
    ...
    # GENUINE sign-off: distinct reviewers, force-close SIGNED phases vetoed by pending
    # items (a reviewer with undecided work cannot have genuinely signed). ReviewerRecords
    # carries the item-derived per-reviewer pending.
    $pendingNamesExec = @()
    ...
    $gsoExec = Get-SPGenuineReviewerSignOff -Reviewers $allRevw -Roster @() -PendingReviewerNames $pendingNamesExec
    $signed = [int]$gsoExec.Signed
    $totRev = [int]$gsoExec.Total
...
# -- Newly Decided: Genuine State Changes from Entitlement State Database --
$ndCnt = $tracking.Entitlement.NewlyDecided.Count
$ndLabel = "Newly Decided -- Genuine State Changes ($ndCnt items)"
# First-run detection via the tracking result, NOT PriorSnapshot (which the
# orchestrator intentionally returns empty on every run -- the old check rendered
# 'first run, no prior state' forever, a false statement on an evidence document).
...
[void]$sb.AppendLine('<div class="section"><h2>Entitlement State Summary</h2>')
$esAppr = [int]$tracking.Entitlement.StateSummary['APPROVE']; $esRev = [int]$tracking.Entitlement.StateSummary['REVOKE']
$esPend = [int]$tracking.Entitlement.StateSummary['PENDING']; $esUnd = [int]$tracking.Entitlement.StateSummary['UNDECIDED']
$esTotal = $esAppr + $esRev + $esPend + $esUnd
$esApprPct = if ($esTotal -gt 0) { [math]::Round($esAppr / $esTotal * 100, 1) } else { 0 }
$esRevPct  = if ($esTotal -gt 0) { [math]::Round($esRev / $esTotal * 100, 1) } else { 0 }
$esPendPct = if ($esTotal -gt 0) { [math]::Round($esPend / $esTotal * 100, 1) } else { 0 }
$esUndPct  = if ($esTotal -gt 0) { [math]::Round($esUnd / $esTotal * 100, 1) } else { 0 }
[void]$sb.AppendLine('<p style="color:#777;font-size:11px;margin:0 0 12px">Current honest decision state for all in-scope entitlement-identity pairs. Sourced from the persistent entitlement state database. UNDECIDED = ISC auto-closed (idNowAutoApproved), reviewer never acted.</p>')
[void]$sb.AppendLine('<div style="display:flex;flex-wrap:wrap;gap:16px 32px;margin-bottom:16px">')
[void]$sb.AppendLine('<div style="text-align:center"><span style="font-size:28px;font-weight:bold;color:#339933">' + ('{0:N0}' -f $esAppr) + '</span><br><span style="font-size:11px;color:#777">Approved (' + $esApprPct + '%)</span></div>')
[void]$sb.AppendLine('<div style="text-align:center"><span style="font-size:28px;font-weight:bold;color:#CC3333">' + ('{0:N0}' -f $esRev) + '</span><br><span style="font-size:11px;color:#777">Revoked (' + $esRevPct + '%)</span></div>')
[void]$sb.AppendLine('<div style="text-align:center"><span style="font-size:28px;font-weight:bold;color:#FF8800">' + ('{0:N0}' -f $esPend) + '</span><br><span style="font-size:11px;color:#777">Pending (' + $esPendPct + '%)</span></div>')
[void]$sb.AppendLine('<div style="text-align:center"><span style="font-size:28px;font-weight:bold;color:#9a6700">' + ('{0:N0}' -f $esUnd) + '</span><br><span style="font-size:11px;color:#777">Undecided (' + $esUndPct + '%)</span></div>')
[void]$sb.AppendLine('<div style="text-align:center"><span style="font-size:28px;font-weight:bold;color:#264d73">' + ('{0:N0}' -f $esTotal) + '</span><br><span style="font-size:11px;color:#777">Total In-Scope</span></div>')
[void]$sb.AppendLine('</div>')
# State file metadata
[void]$sb.AppendLine('<table style="font-size:11px;color:#777;border-collapse:collapse">')
[void]$sb.AppendLine('<tr><td style="padding:2px 8px 2px 0;font-weight:bold">State Records</td><td style="padding:2px 0">' + ('{0:N0}' -f $tracking.Entitlement.Total) + ' total (' + ('{0:N0}' -f $tracking.Entitlement.StateNew) + ' new this run, ' + ('{0:N0}' -f $tracking.Entitlement.StateChanged) + ' state changes)</td></tr>')
[void]$sb.AppendLine('<tr><td style="padding:2px 8px 2px 0;font-weight:bold">Dropped from Scope</td><td style="padding:2px 0">' + ('{0:N0}' -f $tracking.Entitlement.DroppedFromScope.Count) + ' items no longer in any campaign</td></tr>')
[void]$sb.AppendLine('</table>')
[void]$sb.AppendLine('</div>')

# ---- Dropped from Scope ----
if ($tracking.Entitlement.DroppedFromScope.Count -gt 0) {
    $drpCnt = $tracking.Entitlement.DroppedFromScope.Count
    [void]$sb.AppendLine('<div class="section"><h2>Dropped from Scope</h2>')
    [void]$sb.AppendLine('<p style="color:#777;font-size:11px;margin:0 0 8px">Entitlement-identity pairs that were present in prior campaigns but are no longer in any current campaign scope. These records are preserved in the state database for 90 days.</p>')
    [void]$sb.AppendLine("<details><summary style='font-weight:bold;font-size:12px;margin-bottom:4px'>$drpCnt item(s) dropped from scope</summary>")
    [void]$sb.AppendLine('<table class="report"><thead><tr><th>Identity</th><th>Access Name</th><th>Source</th><th>Last Decision</th><th>Last Reviewer</th><th>First Seen</th><th>Campaigns</th><th>Privileged</th></tr></thead><tbody>')
    $drpSorted = @($tracking.Entitlement.DroppedFromScope | Sort-Object @{ Expression = { [string]$_.identityName } })
    foreach ($drp in $drpSorted) {
        $drpIdent   = ConvertTo-SafeHtml ([string]$drp.identityName)
        $drpAccess  = ConvertTo-SafeHtml ([string]$drp.accessName)
        $drpSource  = ConvertTo-SafeHtml ([string]$drp.sourceName)
        $drpDec     = ConvertTo-SafeHtml ([string]$drp.currentDecision)
        $drpReview  = ConvertTo-SafeHtml ([string]$drp.currentReviewer)
        $drpFirst   = ConvertTo-SafeHtml ([string]$drp.firstSeenDate)
        $drpCamps   = [int]$drp.campaignsObserved
        $drpPriv    = $false; try { $drpPriv = [bool]$drp.privileged } catch { }
        $drpPrivBadge = if ($drpPriv) { '<span class="badge badge-priv">PRIV</span>' } else { '' }
        $drpDecCls  = switch ([string]$drp.currentDecision) { 'APPROVE' { 's-green' } 'REVOKE' { 's-red' } 'UNDECIDED' { 's-amber' } default { 's-amber' } }
        [void]$sb.AppendLine('<tr><td>' + $drpIdent + '</td><td>' + $drpAccess + '</td><td>' + $drpSource + '</td><td class="' + $drpDecCls + '">' + $drpDec + '</td><td>' + $drpReview + '</td><td>' + $drpFirst + '</td><td style="text-align:right">' + $drpCamps + '</td><td>' + $drpPrivBadge + '</td></tr>')
    }
    [void]$sb.AppendLine('</tbody></table></details>')
    [void]$sb.AppendLine('</div>')
}
...
# ---- Footer ----
[void]$sb.AppendLine('<div class="footer">SailPoint ISC Governance Toolkit &middot; Daily Evidence Report v4g &middot; Generated: ' + (ConvertTo-SafeHtml $genStr) + ' &middot; CorrelationID: ' + (ConvertTo-SafeHtml $correlationID) + ' &middot; ' + $campaignAudits.Count + ' campaign(s) &middot; State: ' + ('{0:N0}' -f $tracking.Entitlement.Total) + ' records</div>')
[void]$sb.AppendLine('</div></body></html>')
    $htmlContent = $sb.ToString()
    $freshTag = if ($NoCache) { '_fresh' } else { '' }
    $htmlFileName = 'daily-evidence-v4g-{0}{1}.html' -f $startTime.ToString('yyyyMMdd-HHmmss'), $freshTag
...
try {
    ...
    $auditFile = Join-Path $effectiveOutputPath 'daily-evidence-v4g-audit.jsonl'
    ...
    Write-SPLog -Message "Audit trail write failed: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4g' -Action 'AuditTrailError' -CorrelationID $correlationID
}

Write-SPLog -Message "Invoke-SPDailyEvidenceReport completed: ExitCode=$worstExitCode Duration=$durationStr" `
    -Severity INFO -Component 'DailyEvidenceV4g' -Action 'Complete' -CorrelationID $correlationID
