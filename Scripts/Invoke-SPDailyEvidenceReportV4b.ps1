...
<#
.SYNOPSIS
    Generates the daily certification evidence report (v4b) -- a focused accountability and
    delta-aware evidence artifact (output: daily-evidence-v4b-*.html).
.DESCRIPTION
...
# ---- Footer ----
[void]$sb.AppendLine('<div class="footer">SailPoint ISC Governance Toolkit &middot; Daily Evidence Report v4b &middot; Generated: ' + (ConvertTo-SafeHtml $genStr) + ' &middot; CorrelationID: ' + (ConvertTo-SafeHtml $correlationID) + ' &middot; ' + $campaignAudits.Count + ' campaign(s)</div>')
[void]$sb.AppendLine('</div></body></html>')
    $htmlContent = $sb.ToString()
    $freshTag = if ($NoCache) { '_fresh' } else { '' }
    $htmlFileName = 'daily-evidence-v4b-{0}{1}.html' -f $startTime.ToString('yyyyMMdd-HHmmss'), $freshTag
...
try {
    ...
    $auditFile = Join-Path $effectiveOutputPath 'daily-evidence-v4b-audit.jsonl'
