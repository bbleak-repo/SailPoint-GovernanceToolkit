...
.DESCRIPTION
    ...
    presents the certification evidence that matters, in this order:
      ...
      Decision Summary             collapsible Approved / Revoked (open) / Pending registers

    No due-date signals (report-generated date is used). Day-over-day / scope-change deltas are
...
foreach ($audit in $campaignAudits) {
    ...
    # Build unique reviewer set from certifications (keyed by reviewer identity ID
    # to avoid double-counting when a reviewer is both primary AND reassigned).
...
foreach ($audit in $campaignAudits) {
    ...
    if ($reassigned.Count -eq 0) { [void]$sb.AppendLine('<tr><td colspan="7" style="color:#777;font-style:italic">No reassignments recorded.</td></tr>') }
    else { foreach ($rr in @($reassigned | Sort-Object Name)) {
        $so = & $fmtDt ([string]$rr.SignOffDate)
...
$cats = @(
    @{ Label = 'Approved'; Cls = 's-green'; Items = $allApproved; Open = $false },
    @{ Label = 'Revoked'; Cls = 's-red'; Items = $allRevoked; Open = $true },
    @{ Label = 'Pending'; Cls = 's-amber'; Items = $allPending; Open = $false }
)
foreach ($cat in $cats) {
...
# ---- Footer ----
[void]$sb.AppendLine('<div class="footer">SailPoint ISC Governance Toolkit &middot; Daily Evidence Report v2 &middot; Generated: ' + (ConvertTo-SafeHtml $genStr) + ' &middot; CorrelationID: ' + (ConvertTo-SafeHtml $correlationID) + ' &middot; ' + $campaignAudits.Count + ' campaign(s)</div>')
[void]$sb.AppendLine('</div></body></html>')
    $htmlContent = $sb.ToString()
    $htmlFileName = 'daily-evidence-v2-{0}.html' -f $startTime.ToString('yyyyMMdd-HHmmss')
...
try {
    ...
    $auditFile = Join-Path $effectiveOutputPath 'daily-evidence-v2-audit.jsonl'
