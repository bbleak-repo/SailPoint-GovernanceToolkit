...
<#
.SYNOPSIS
    Generates the daily certification evidence report (v4c) -- v4 with persistent entitlement
    state database for accurate "Newly Decided" detection (output: daily-evidence-v4c-*.html).
.DESCRIPTION
    V4c is a fork of V4 that adds a persistent entitlement state database
    ...
    comments="idNowAutoApproved", making two-campaign diffs unreliable. V4c tracks
    ...
    V4c integrates the scope-diff engine (same as V3/V4) and the entitlement state database.
    Use -NoCapture to re-render from existing snapshots without capturing a new one.

    V4 remains available unchanged. Use V4c when you need accurate Newly Decided tracking.
...
Write-Host '  Daily Evidence Report (v4c)' -ForegroundColor Cyan
...
try {
    ...
    if ($currentCampaigns.Count -eq 0) {
        ...
        Write-SPLog -Message "No campaigns found in last $effectiveDaysBack day(s)" `
            -Severity WARN -Component 'DailyEvidenceV4c' -Action 'NoCampaigns' -CorrelationID $correlationID
        Write-Host ''
        exit 1
    }
    ...
    foreach ($campaign in $currentCampaigns) {
        ...
        try {
            ...
            Write-SPLog -Message "Campaign audit build failed for ${campName}: $($_.Exception.Message)" `
                -Severity WARN -Component 'DailyEvidenceV4c' -Action 'AuditBuildError' -CorrelationID $correlationID
        }
    }
    ...
    Write-SPLog -Message "Campaign fetch exception: $($_.Exception.Message)" `
        -Severity ERROR -Component 'DailyEvidenceV4c' -Action 'CampaignFetchError' -CorrelationID $correlationID
    $worstExitCode = 5
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "Scope-diff exception: $($_.Exception.Message)" -Severity ERROR -Component 'DailyEvidenceV4c' -Action 'ScopeDiffError' -CorrelationID $correlationID
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "State tracking exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4c' -Action 'StateTrackingError' -CorrelationID $correlationID
}
...
try {
    ...
    Write-SPLog -Message "Campaign completion KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4c' -Action 'CompletionError' -CorrelationID $correlationID
...
try {
    ...
    Write-SPLog -Message "Overdue reviews KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4c' -Action 'OverdueError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "Revocations KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4c' -Action 'RevocationError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "Remediation SLA KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4c' -Action 'SlaError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "High-risk exposure KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4c' -Action 'HighRiskError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "Reviewer health KPI exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4c' -Action 'ReviewerHealthError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
try {
    ...
    Write-SPLog -Message "Confidence score exception: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4c' -Action 'ConfidenceError' -CorrelationID $correlationID
    $worstExitCode = [math]::Max($worstExitCode, 1)
}
Write-Host ''
...
if ($priorStateSnapshot.Count -eq 0) { $ndLabel = "Newly Decided ($ndCnt items) -- first run, no prior state for comparison" }
...
# ---- Footer ----
[void]$sb.AppendLine('<div class="footer">SailPoint ISC Governance Toolkit &middot; Daily Evidence Report v4c &middot; Generated: ' + (ConvertTo-SafeHtml $genStr) + ' &middot; CorrelationID: ' + (ConvertTo-SafeHtml $correlationID) + ' &middot; ' + $campaignAudits.Count + ' campaign(s) &middot; State: ' + ('{0:N0}' -f $tracking.Entitlement.Total) + ' records</div>')
[void]$sb.AppendLine('</div></body></html>')
    $htmlContent = $sb.ToString()
    $freshTag = if ($NoCache) { '_fresh' } else { '' }
    $htmlFileName = 'daily-evidence-v4c-{0}{1}.html' -f $startTime.ToString('yyyyMMdd-HHmmss'), $freshTag
...
try {
    ...
    $auditFile = Join-Path $effectiveOutputPath 'daily-evidence-v4c-audit.jsonl'
    ...
    Write-SPLog -Message "Audit trail write failed: $($_.Exception.Message)" `
        -Severity WARN -Component 'DailyEvidenceV4c' -Action 'AuditTrailError' -CorrelationID $correlationID
}
