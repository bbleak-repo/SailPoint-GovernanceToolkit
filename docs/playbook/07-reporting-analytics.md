...
| **Daily Decision Trend** | Three adaptive charts on one aligned date axis -- combined paired red/green, revoked-only, new-scope-only -- plus a raw per-day numbers table with CUMULATIVE columns that reconcile back to the source reports' register counts (a report showing 35 the day after 28 = 7 first-sightings; cumulative reads 35). Every report day appears on the axis (a zero bar = report ran, nothing decided); only days with neither a report nor activity are absent. Bar width and date-label density adapt to the window (15-90+ days); labels rotate below the axis |
| **Re-Approved After Revoke (flops)** | Grants revoked earlier in the window and approved again later (approve -> revoke -> approve), matched by identity + access + source across the deduped registers. Also explains a SHRINKING Revoked count between two reports: re-approved items leave the current-state register. Both events must fall inside the window; the V4g/V8 state DB tracks re-approvals across any span |
| **Re-Revoked Grants (repeat revocations)** | The same grant revoked on more than one decision day -- revoked access that came back (failed remediation, re-provisioning, or a revoke -> approve -> revoke flip). Distinct revoke events are NEVER de-duplicated (the dedupe key includes the decision date); this table makes the repeat pattern visible. Intra-report duplicate rows (the same key twice in ONE file -- possible cache duplication) are flagged as a data-quality warning instead of being silently collapsed |
